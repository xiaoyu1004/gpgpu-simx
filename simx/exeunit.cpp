...
void LsuUnit::tick() {
    ...
    for (uint32_t t = 0; t < num_threads_; ++t) {
        ...
        DT(3,
           "dcache-rsp: tag=" << mem_rsp.tag << ", type=" << trace->lsu.type << ", tid=" << t
                              << ", " << *trace);
        assert(entry.second);
    ...
    for (uint32_t t = 0; t < num_threads_; ++t) {
        ...
        auto trace    = entry.first;
        DT(3,
           "smem-rsp: tag=" << mem_rsp.tag << ", type=" << trace->lsu.type << ", tid=" << t << ", "
                            << *trace);
        assert(entry.second);
        --entry.second;  // track remaining blocks
        if (0 == entry.second) {
    ...
    for (uint32_t t = 0; t < num_threads_; ++t) {
        ...
        if (type == AddrType::Shared) {
            ...
            DT(3,
               "smem-req: addr=" << std::hex << mem_addr.addr << ", tag=" << tag << ", type="
                                 << trace->lsu.type << ", tid=" << t << ", " << *trace);
        } else {
            dcache_req_port.send(mem_req, 2);
            DT(3,
               "dcache-req: addr=" << std::hex << mem_addr.addr << ", tag=" << tag
                                   << ", type=" << trace->lsu.type << ", tid=" << t
                                   << ", nc=" << mem_req.non_cacheable << ", " << *trace);
        }
...
void AluUnit::tick() {
    ...
    case AluType::CMOV: Output.send(trace, 1); break;
    case AluType::IMUL: Output.send(trace, LATENCY_IMUL + 1); break;
    case AluType::IDIV: Output.send(trace, XLEN + 1); break;
    default: std::abort();
    }
...
void FpuUnit::tick() {
    ...
    case FpuType::FNCP: Output.send(trace, 2); break;
    case FpuType::FMA: Output.send(trace, LATENCY_FMA + 1); break;
    case FpuType::FDIV: Output.send(trace, LATENCY_FDIV + 1); break;
    case FpuType::FSQRT: Output.send(trace, LATENCY_FSQRT + 1); break;
    case FpuType::FCVT: Output.send(trace, LATENCY_FCVT + 1); break;
    default: std::abort();
    }
...
void GpuUnit::reset() { pending_tex_reqs_.clear(); }
...
bool GpuUnit::processTexRequest(pipeline_trace_t* trace) {
    ...
    for (uint32_t t = 0; t < num_threads_; ++t) {
        ...
        for (auto& mem_addr : trace->mem_addrs.at(t)) {
            ...
            DT(3,
               "tex-req: addr=" << std::hex << mem_addr.addr << ", tag=" << tag << ", tid=" << t
                                << ", " << trace);
