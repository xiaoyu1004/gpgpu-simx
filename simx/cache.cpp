...
struct mshr_entry_t : public bank_req_t {
    ...
    mshr_entry_t(uint32_t size = 0) : bank_req_t(size), block_id(0) {}
};
...
class MSHR {
 ...
 public:
    MSHR(uint32_t size) : entries_(size), size_(0) {}
    ...
    int lookup(const bank_req_t& bank_req) {
        for (uint32_t i = 0, n = entries_.size(); i < n; ++i) {
            ...
            if (entry.valid && entry.set_id == bank_req.set_id && entry.tag == bank_req.tag) {
                return i;
            }
        }
        return -1;
    ...
    mshr_entry_t& replay(uint32_t id) {
        ...
        for (auto& entry : entries_) {
            if (entry.valid && entry.set_id == root_entry.set_id && entry.tag == root_entry.tag) {
...
struct bank_t {
    ...
    bank_t(const Cache::Config& config, const params_t& params)
        : sets(params.sets_per_bank, params.blocks_per_set), mshr(config.mshr_size) {}
 ...
 public:
    Impl(Cache* simobject, const Config& config)
        ...
        if (config.num_banks > 1) {
            mem_switch_ = Switch<MemReq, MemRsp>::Create(
                "mem_arb", ArbiterType::RoundRobin, config.num_banks);
    ...
    void tick() {
        ...
        for (uint32_t req_id = 0, n = config_.num_inputs; req_id < n; ++req_id) {
            ...
            if (pipeline_req.valid && pipeline_req.mshr_replay) {
                // stall
                continue;
            }
            ...
            if ((!core_req.write || !config_.write_through) && bank.mshr.full()) {
            ...
            if (pipeline_req.valid) {
                // check port conflict
                if (pipeline_req.write != core_req.write || pipeline_req.set_id != set_id ||
                    pipeline_req.tag != tag || pipeline_req.infos[port_id].valid) {
                ...
                if (hit) {
                    ...
                    if (pipeline_req.write) {
                        ...
                        if (config_.write_through) {
                            ...
                            MemReq mem_req;
                            mem_req.addr =
                                params_.mem_addr(bank_id, pipeline_req.set_id, hit_block.tag);
                            mem_req.write   = true;
                    ...
                    if (!pipeline_req.write || config_.write_reponse) {
                        for (auto& info : pipeline_req.infos) {
                            MemRsp core_rsp{info.req_tag, pipeline_req.core_id, pipeline_req.uuid};
                            simobject_->CoreRspPorts.at(info.req_id)
                                .send(core_rsp, config_.latency);
                            DT(3, simobject_->name() << "-" << core_rsp);
                        }
                    }
                } else {
                    ...
                    if (!found_free_block && !config_.write_through) {
                        ...
                        if (repl_block.dirty) {
                            MemReq mem_req;
                            mem_req.addr =
                                params_.mem_addr(bank_id, pipeline_req.set_id, repl_block.tag);
                            mem_req.write   = true;
                            mem_req.core_id = pipeline_req.core_id;
                            mem_req_ports_.at(bank_id).send(mem_req, 1);
                            DT(3, simobject_->name() << "-" << mem_req);
                    ...
                    if (pipeline_req.write && config_.write_through) {
                        // forward write request to memory
                        {
                            MemReq mem_req;
                            mem_req.addr =
                                params_.mem_addr(bank_id, pipeline_req.set_id, pipeline_req.tag);
                            mem_req.write   = true;
                            mem_req.core_id = pipeline_req.core_id;
                            mem_req.uuid    = pipeline_req.uuid;
                            mem_req_ports_.at(bank_id).send(mem_req, 1);
                        ...
                        if (config_.write_reponse) {
                            for (auto& info : pipeline_req.infos) {
                                MemRsp core_rsp{
                                    info.req_tag, pipeline_req.core_id, pipeline_req.uuid};
                                simobject_->CoreRspPorts.at(info.req_id)
                                    .send(core_rsp, config_.latency);
                                DT(3, simobject_->name() << "-" << core_rsp);
                            }
...
Cache::~Cache() { delete impl_; }

void Cache::reset() { impl_->reset(); }

void Cache::tick() { impl_->tick(); }

const Cache::PerfStats& Cache::perf_stats() const { return impl_->perf_stats(); }