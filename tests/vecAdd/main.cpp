...
uint64_t shuffle(int i, uint64_t value) { return (value << i) | (value & ((1 << i) - 1)); }
...
int run_memcopy_test(uint32_t dev_addr, uint64_t value, int num_blocks) {
    ...
    for (int i = 0; i < num_blocks_8; ++i) {
        ...
        if (curr != ref) {
            std::cout << "error at 0x" << std::hex << (dev_addr + 8 * i) << ": actual 0x" << curr
...
int run_kernel_test(const kernel_arg_t& kernel_arg, uint32_t buf_size, uint32_t num_points) {
    int errors = 0;
