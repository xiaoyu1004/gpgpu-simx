...
class Processor::Impl {
 ...
 public:
    Impl(const ArchDef& arch)
        : cores_(arch.num_cores()), l2caches_(NUM_CLUSTERS), l2_mem_switches_(NUM_CLUSTERS) {
        ...
        if (L3_ENABLE) {
            ...
            l3_mem_switch_ =
                Switch<MemReq, MemRsp>::Create("l3_arb", ArbiterType::RoundRobin, NUM_CLUSTERS);
        ...
        for (uint32_t i = 0; i < NUM_CLUSTERS; ++i) {
            ...
            if (L2_ENABLE) {
                ...
                l2_mem_switch       = Switch<MemReq, MemRsp>::Create(
                    "l2_arb", ArbiterType::RoundRobin, cores_per_cluster);
    ...
    ~Impl() { SimPlatform::instance().finalize(); }
...
Processor::Processor(const ArchDef& arch) : impl_(new Impl(arch)) {}

Processor::~Processor() { delete impl_; }

void Processor::attach_ram(RAM* mem) { impl_->attach_ram(mem); }

int Processor::run() { return impl_->run(); }