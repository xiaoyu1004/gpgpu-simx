...
uint32_t rv_fmin_s(uint32_t a, uint32_t b, uint32_t* fflags) {
    ...
    if (isNaNF32UI(a) && isNaNF32UI(b)) {
        ...
        if ((f32_lt_quiet(fa, fb) || (f32_eq(fa, fb) && (a & F32_SIGN))) || isNaNF32UI(b)) {
            r = a;
        } else {
            r = b;
        }
...
uint64_t rv_fmin_d(uint64_t a, uint64_t b, uint32_t* fflags) {
    ...
    if (isNaNF64UI(a) && isNaNF64UI(b)) {
        ...
        if ((f64_lt_quiet(fa, fb) || (f64_eq(fa, fb) && (a & F64_SIGN))) || isNaNF64UI(b)) {
            r = a;
        } else {
            r = b;
        }
...
uint32_t rv_fmax_s(uint32_t a, uint32_t b, uint32_t* fflags) {
    ...
    if (isNaNF32UI(a) && isNaNF32UI(b)) {
        ...
        auto fa = to_float32_t(a);
        auto fb = to_float32_t(b);
        if ((f32_lt_quiet(fb, fa) || (f32_eq(fb, fa) && (b & F32_SIGN))) || isNaNF32UI(b)) {
            r = a;
        } else {
            r = b;
        }
...
uint64_t rv_fmax_d(uint64_t a, uint64_t b, uint32_t* fflags) {
    ...
    if (isNaNF64UI(a) && isNaNF64UI(b)) {
        ...
        auto fa = to_float64_t(a);
        auto fb = to_float64_t(b);
        if ((f64_lt_quiet(fb, fa) || (f64_eq(fb, fa) && (b & F64_SIGN))) || isNaNF64UI(b)) {
            r = a;
        } else {
            r = b;
        }
...
uint32_t rv_fclss_s(uint32_t a) {
    ...
    uint32_t r =
        (sign && infOrNaN && fracZero) << 0 | (sign && !infOrNaN && !subnormOrZero) << 1 |
        (sign && subnormOrZero && !fracZero) << 2 | (sign && subnormOrZero && fracZero) << 3 |
        (!sign && infOrNaN && fracZero) << 7 | (!sign && !infOrNaN && !subnormOrZero) << 6 |
        (!sign && subnormOrZero && !fracZero) << 5 | (!sign && subnormOrZero && fracZero) << 4 |
