...
void Warp::eval(pipeline_trace_t* trace) {
    ...
    if (!instr) {
        std::cout << std::hex << "Error: invalid instruction 0x" << instr_code << ", at PC=" << PC_
                  << std::endl;
        std::abort();
    }
    ...
    for (uint32_t i = 0; i < core_->arch().num_regs(); ++i) {
        ...
        for (uint32_t j = 0; j < core_->arch().num_threads(); ++j) {
            DPN(4,
                ' ' << std::setfill('0') << std::setw(XLEN / 4) << std::hex
                    << ireg_file_.at(j).at(i) << std::setfill(' ') << ' ');
        }
        DPN(4, '|');
        // Floating point register file
        for (uint32_t j = 0; j < core_->arch().num_threads(); ++j) {
            DPN(4,
                ' ' << std::setfill('0') << std::setw(16) << std::hex << freg_file_.at(j).at(i)
                    << std::setfill(' ') << ' ');
        }
        DPN(4, std::endl);
    }
