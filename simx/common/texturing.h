...
T Clamp(Fixed<F, T> fx, WrapMode mode) {
    ...
    case WrapMode::Clamp:
        return (fx.data() < 0) ? 0
                               : ((fx.data() > Fixed<F, T>::MASK) ? Fixed<F, T>::MASK : fx.data());
...
inline void Unpack8888(TexFormat format, uint32_t texel, uint32_t* lo, uint32_t* hi) {
    uint32_t r, g, b, a;
    switch (format) {
...
inline uint32_t Pack8888(uint32_t lo, uint32_t hi) { return (hi << 8) | lo; }
...
void TexAddressLinear(Fixed<F, T> fu,
    ...
    *beta  = y0s & 0xff;
    ...
    // printf("*** fu=0x%x, fv=0x%x, u0=0x%x, u1=0x%x, v0=0x%x, v1=0x%x, x0=0x%x, x1=0x%x, y0=0x%x,
    // y1=0x%x, addr00=0x%x, addr01=0x%x, addr10=0x%x, addr11=0x%x\n", fu.data(), fv.data(), u0, u1,
    // v0, v1, x0, x1, y0, y1, *addr00, *addr01, *addr10, *addr11);
}
...
void TexAddressPoint(Fixed<F, T> fu,
    ...
    *addr = x + (y << log_width);
    ...
    // fv.data(), u, v, x, y, *addr);
}

inline uint32_t TexFilterLinear(TexFormat format,
                                uint32_t texel00,
                                uint32_t texel01,
                                uint32_t texel10,
                                uint32_t texel11,
