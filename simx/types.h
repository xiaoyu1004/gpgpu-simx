...
enum class RegType { None, Integer, Float, Vector };
...
inline AddrType get_addr_type(Word addr, uint32_t size) {
    __unused(size);
    if (SM_ENABLE) {
        if (addr >= (SMEM_BASE_ADDR - SMEM_SIZE) && addr < SMEM_BASE_ADDR) {
...
enum class ArbiterType { Priority, RoundRobin };
...
inline std::ostream& operator<<(std::ostream& os, const MemReq& req) {
    ...
    os << "addr=" << std::hex << req.addr << std::dec << ", tag=" << req.tag
       << ", core_id=" << req.core_id;
    os << " (#" << std::dec << req.uuid << ")";
    return os;
}
...
struct MemRsp {
    ...
    MemRsp(uint64_t _tag = 0, uint32_t _core_id = 0, uint64_t _uuid = 0)
        : tag(_tag), core_id(_core_id), uuid(_uuid) {}
};
...
class HashTable {
 ...
 public:
    HashTable(uint32_t capacity) : entries_(capacity), size_(0) {}
    ...
    bool contains(uint32_t index) const { return entries_.at(index).first; }
