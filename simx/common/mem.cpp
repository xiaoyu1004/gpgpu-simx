...
RamMemDevice::RamMemDevice(const char* filename, uint32_t wordSize) : wordSize_(wordSize) {
...
void RamMemDevice::read(void* data, uint64_t addr, uint64_t size) {
    ...
    if ((addr & (wordSize_ - 1)) || (addr_end & (wordSize_ - 1)) ||
...
void MemoryUnit::attach(MemDevice& m, uint64_t start, uint64_t end) { decoder_.map(start, end, m); }
...
RAM::RAM(uint32_t page_size)
    : size_(0), page_bits_(log2ceil(page_size)), last_page_(nullptr), last_page_index_(0) {
    ...
    while (true) {
        if (line[0] == ':') {
            ...
            case 0:
                ...
                break;
            case 2: offset = hToI(line + 9, 4) << 4; break;
            case 4: offset = hToI(line + 9, 4) << 16; break;
            default: break;
            }
        }
        while (*line != '\n' && size != 0) {
