...
#define vx_tex(unit, u, v, lod)                                      \
    ({                                                               \
        unsigned __r;                                                \
        __asm__ __volatile__(".insn r4 0x5b, 0, %1, %0, %2, %3, %4"  \
                             : "=r"(__r)                             \
                             : "i"(unit), "r"(u), "r"(v), "r"(lod)); \
        __r;                                                         \
    })
...
inline void vx_split(int predicate) { asm volatile(".insn s 0x6b, 2, x0, 0(%0)" ::"r"(predicate)); }

// Join
inline void vx_join() { asm volatile(".insn s 0x6b, 3, x0, 0(x0)"); }
...
// Prefetch
inline void vx_prefetch(unsigned addr) { asm volatile(".insn s 0x6b, 5, x0, 0(%0)" ::"r"(addr)); }
...
inline void vx_fence() { asm volatile("fence iorw, iorw"); }
