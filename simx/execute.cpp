...
inline bool is_nan_boxed(uint64_t value) { return (uint32_t(value >> 32) == 0xffffffff); }
...
void Warp::execute(const Instr& instr, pipeline_trace_t* trace) {
        ...
        if ((opcode == L_INST) || (opcode == FL && func3 == 2) || (opcode == FL && func3 == 3)) {
            ...
            case 6: {
                for (uint32_t i = 0; i < vl_; i++) {
                    ...
                    *result_ptr      = mem_data;
                    DP(4,
                       "LOAD MEM: ADDRESS=0x" << std::hex << mem_addr << ", DATA=0x" << mem_data);
                }
                break;
            }
            default: std::abort();
    ...
    case FS: {
        ...
        if ((opcode == S_INST) || (opcode == FS && func3 == 2) || (opcode == FS && func3 == 3)) {
            ...
            for (uint32_t t = 0; t < num_threads; ++t) {
                ...
                case 1:
                case 2:
                case 3: core_->dcache_write(&mem_data, mem_addr, mem_bytes); break;
                default: std::abort();
                }
            }
        } else {
            for (uint32_t i = 0; i < vl_; i++) {
                ...
                case 6: {
                    ...
                    DP(4,
                       "STORE MEM: ADDRESS=0x" << std::hex << mem_addr << ", DATA=0x" << mem_data);
                    break;
                }
                default: std::abort();
                }
        ...
        for (uint32_t t = 0; t < num_threads; ++t) {
            ...
            case FMADD:
                if (func2)
                    // RV32D: FMADD.D
                    rddata[t].f =
                        rv_fmadd_d(rsdata[t][0].f, rsdata[t][1].f, rsdata[t][2].f, frm, &fflags);
                else
                    // RV32F: FMADD.S
                    rddata[t].f = nan_box(
                        rv_fmadd_s(rsdata[t][0].f, rsdata[t][1].f, rsdata[t][2].f, frm, &fflags));
                break;
            case FMSUB:
                if (func2)
                    // RV32D: FMSUB.D
                    rddata[t].f =
                        rv_fmsub_d(rsdata[t][0].f, rsdata[t][1].f, rsdata[t][2].f, frm, &fflags);
                else
                    // RV32F: FMSUB.S
                    rddata[t].f = nan_box(
                        rv_fmsub_s(rsdata[t][0].f, rsdata[t][1].f, rsdata[t][2].f, frm, &fflags));
                break;
            case FMNMADD:
                if (func2)
                    // RV32D: FNMADD.D
                    rddata[t].f =
                        rv_fnmadd_d(rsdata[t][0].f, rsdata[t][1].f, rsdata[t][2].f, frm, &fflags);
                else
                    // RV32F: FNMADD.S
                    rddata[t].f = nan_box(
                        rv_fnmadd_s(rsdata[t][0].f, rsdata[t][1].f, rsdata[t][2].f, frm, &fflags));
                break;
            case FMNMSUB:
                if (func2)
                    // RV32D: FNMSUB.D
                    rddata[t].f =
                        rv_fnmsub_d(rsdata[t][0].f, rsdata[t][1].f, rsdata[t][2].f, frm, &fflags);
                else
                    // RV32F: FNMSUB.S
                    rddata[t].f = nan_box(
                        rv_fnmsub_s(rsdata[t][0].f, rsdata[t][1].f, rsdata[t][2].f, frm, &fflags));
                break;
            default: break;
            }
            update_fcrs(fflags, core_, t, id_);
    ...
    case VSET: {
        ...
        case 7: {
            ...
            DP(3,
               "lmul:" << vtype_.vlmul << " sew:" << vtype_.vsew << " ediv: " << vtype_.vediv
                       << "rsrc_" << rsdata[0][0].i << "VLMAX" << VLMAX);
