...
class SimPort : public SimPortBase {
 public:
    ...
    SimPort(SimObjectBase* module) : SimPortBase(module), peer_(nullptr), tx_cb_(nullptr) {}
    ...
    void unbind() { peer_ = nullptr; }

    bool connected() const { return (peer_ != nullptr); }

    SimPort* peer() const { return peer_; }

    bool empty() const { return queue_.empty(); }

    const Pkt& front() const { return queue_.front(); }

    Pkt& front() { return queue_.front().pkt; }

    const Pkt& back() const { return queue_.back(); }

    Pkt& back() { return queue_.back().pkt; }
    ...
    void tx_callback(const TxCallback& callback) { tx_cb_ = callback; }
...
class SimCallEvent : public SimEventBase {
 public:
    void fire() const override { func_(pkt_); }

    typedef std::function<void(const Pkt&)> Func;

    SimCallEvent(const Func& func, const Pkt& pkt, uint64_t time)
        : SimEventBase(time), func_(func), pkt_(pkt) {}

    void* operator new(size_t /*size*/) { return allocator().allocate(); }
...
class SimPortEvent : public SimEventBase {
 public:
    void fire() const override { const_cast<SimPort<Pkt>*>(port_)->push(pkt_, time_); }

    SimPortEvent(const SimPort<Pkt>* port, const Pkt& pkt, uint64_t time)
        : SimEventBase(time), port_(port), pkt_(pkt) {}
...
class SimObject : public SimObjectBase {
 ...
 private:
    ...
    void do_reset() override { this->impl()->reset(); }

    void do_tick() override { this->impl()->tick(); }
};
...
class SimPlatform {
 public:
    ...
    void release_object(const SimObjectBase::Ptr& object) { objects_.remove(object); }

    template <typename Pkt>
    void
    schedule(const typename SimCallEvent<Pkt>::Func& callback, const Pkt& pkt, uint64_t delay) {
        assert(delay != 0);
 ...
 private:
    ...
    virtual ~SimPlatform() { this->clear(); }
    ...
    template <typename U>
    friend class SimPort;
    friend class SimObjectBase;
};

///////////////////////////////////////////////////////////////////////////////

inline SimObjectBase::SimObjectBase(const SimContext&, const char* name) : name_(name) {}
