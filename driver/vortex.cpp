...
class vx_buffer {
 public:
    vx_buffer(uint64_t size, vx_device* device) : size_(size), device_(device) {
    ...
    vx_device* device() const { return device_; }
 ...
 public:
    vx_device()
        : arch_(NUM_CORES * NUM_CLUSTERS, NUM_WARPS, NUM_THREADS),
          ...
          processor_(arch_),
          mem_allocator_(
              ALLOC_BASE_ADDR, ALLOC_BASE_ADDR + LOCAL_MEM_SIZE, RAM_PAGE_SIZE, CACHE_BLOCK_SIZE) {
    ...
    int free_local_mem(uint64_t dev_maddr) { return mem_allocator_.release(dev_maddr); }
    ...
    int upload(const void* src, uint64_t dest_addr, uint64_t size, uint64_t src_offset) {
        ...
        for (int i = 0; i < size; i += 4) {
            printf("mem-write: 0x%x <- 0x%x\n", dest_addr + i, *(uint32_t*)((uint8_t*)src +
        src_offset + i));
        }*/
    ...
    int wait(uint64_t timeout) {
        ...
        for (;;) {
            ...
            if (status == std::future_status::ready || 0 == timeout_sec--)
                break;
        }
        return 0;
    }
...
extern int vx_dev_caps(vx_device_h hdevice, uint32_t caps_id, uint64_t* value) {
    ...
    case VX_CAPS_MAX_CORES: *value = NUM_CORES * NUM_CLUSTERS; break;
    case VX_CAPS_MAX_WARPS: *value = NUM_WARPS; break;
    case VX_CAPS_MAX_THREADS: *value = NUM_THREADS; break;
